...
async function checkRateLimit(env, key, limit, windowSecs) {
  if (!env.RATE_LIMIT) return false; // KV not configured — caller handles
  try {
...
// Returns true (blocked) if KV is missing — fail closed on critical routes
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/auth/login') {
      // Rate limit: max 20 login attempts per IP per 5 minutes
    ...
    if (url.pathname === '/api/tts') {
      // Auth check — must be before IP rate limit to prevent unauthenticated quota drain
      ...
      const { text, voice_id, model_id = 'eleven_multilingual_v2', language_code, natural_speed } = body;
    ...
    if (url.pathname === '/crash') {
      ...
      try {
        const ip = request.headers.get('CF-Connecting-IP') || 'unknown';
        // Rate limit: max 20 crash reports per IP per hour (prevent abuse)
        if (await checkRateLimit(env, `crash:${ip}`, 20, 3600))
          return corsResponse('', 204, origin); // silently drop, don't alert client
        ...
        if (body && env.RATE_LIMIT) {
          ...
          await env.RATE_LIMIT.put(key, JSON.stringify(safe), { expirationTtl: 604800 }); // 7 days
        }
      } catch(e) {}
    ...
    if (url.pathname === '/api/chat' || url.pathname === '/') {
      ...
      // Lock to allowed models — prevent clients from selecting expensive models
      ...
      // Allowlist body fields — strip tools, tool_choice, metadata, etc.
      const safeBody = {
    ...
    if (url.pathname === '/api/memory') {
      if (request.method === 'GET') {
        // Fetch due moments for prefetch (dashboard load)
      ...
      if (request.method === 'POST') {
        // Upsert a memory moment
        if (!userId) return corsResponse(JSON.stringify({ error: 'Auth required' }), 401, origin);
        let body;
        try { body = await request.json(); } catch { return corsResponse(JSON.stringify({ error: 'Invalid JSON' }), 400, origin); }
        const moments = Array.isArray(body) ? body.slice(0, 20) : [body]; // max 20 moments per call
      ...
      if (request.method === 'PATCH') {
        // Update SR state for a moment after session
        if (!userId) return corsResponse(JSON.stringify({ error: 'Auth required' }), 401, origin);
        let body;
        try { body = await request.json(); } catch { return corsResponse(JSON.stringify({ error: 'Invalid JSON' }), 400, origin); }
        ...
        const res = await fetch(
          `${env.SUPABASE_URL}/rest/v1/memory_moments?user_id=eq.${userId}&phrase_norm=eq.${encodeURIComponent(phrase_norm)}&lang=eq.${encodeURIComponent(lang)}&dialect=eq.${encodeURIComponent(dialect)}`,
          {
            method: 'PATCH',
    ...
    if (url.pathname === '/api/stt') {
      ...
      // Sanitize language to BCP-47 alpha codes only (e.g. 'es', 'fr-FR')
      ...
      // Validate file size from the blob itself (Content-Length can be spoofed)
      ...
      outForm.append('response_format', 'verbose_json');  // returns text + word confidence
...
async function handleSignup(request, env, origin) {
  ...
  // Supabase returns 200 + null user when email is already registered (email enumeration protection)
  // Surface a cleaner error so the app can handle it
...
async function handleCreateCheckout(request, env, origin, userId, userEmail) {
  ...
  const params = new URLSearchParams({
    ...
    // Cancel at period end instead of immediate deletion
...
async function handleStripeWebhook(request, env, origin) {
  ...
  // Subscription cancelled — revoke Pro at period end (not immediately)
  if (event.type === 'customer.subscription.updated') {
    const sub = event.data.object;
    // Only revoke when cancel_at_period_end is true AND the period has actually ended
  ...
  // Subscription fully deleted (after period ends)
...
async function verifyStripeSignature(payload, sigHeader, secret) {
  try {
    ...
    // Decode the hex signature to bytes for constant-time comparison
...
// (handled client-side in the app via ?upgrade=cancelled param)
